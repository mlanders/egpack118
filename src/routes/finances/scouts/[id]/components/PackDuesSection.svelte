<script lang="ts">
	...
	function getProgressPercentage(): number {
		return Math.min(100, (duesSummary.totalPaid / duesSummary.packDuesAmount) * 100);
	}
</script>
...
<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
	<div class="flex items-center justify-between mb-6">
		<div>
			<h2 class="text-lg font-semibold text-gray-900">Pack Dues {duesSummary.fiscalYear}</h2>
	...
	{#if duesSummary.duesOverrideReason}
		<div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
			<div class="flex items-start gap-2">
				...
				<div>
					<strong class="text-yellow-900">Dues Override Applied</strong>
					<p class="text-yellow-800 text-sm mt-1">{duesSummary.duesOverrideReason}</p>
				</div>
			</div>
		</div>
	{/if}
	...
	<div class="mb-6">
		<div class="flex justify-between text-sm mb-2">
			<span class="font-medium text-gray-700">
				Paid: <span class="text-green-600">${duesSummary.totalPaid.toFixed(2)}</span>
			</span>
			<span class="font-medium text-gray-700">
				Remaining: <span class="text-red-600">${duesSummary.remaining.toFixed(2)}</span>
			</span>
		</div>
	...
	<div class="mt-6">
		...
		{#if duesSummary.payments.length === 0}
			<p class="text-gray-500 text-center py-8 bg-gray-50 rounded-lg">No payments recorded yet</p>
		{:else}
				...
				<table class="min-w-full divide-y divide-gray-200">
					<thead class="bg-gray-50">
						<tr>
							...
							<th class="px-4 py-2 text-left text-xs font-medium text-gray-600 uppercase">Check #</th>
						...
						{#each duesSummary.payments as payment}
							<tr class="hover:bg-gray-50">
								...
								<td class="px-4 py-3 whitespace-nowrap text-sm">
									<span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium {getMethodBadgeClass(payment.paymentMethod)}">
