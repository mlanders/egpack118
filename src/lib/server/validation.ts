...
export const fiscalYearConfigSchema = z.object({
  fiscalYear: z.string().regex(/^\d{4}-\d{4}$/, "Invalid fiscal year format"),
  packDuesAmount: z.number().positive("Pack dues amount must be positive"),
});

export const packDuesPaymentSchema = z
  .object({
    scoutId: z.number().int().positive("Scout ID must be a positive integer"),
    scoutName: z.string().min(1, "Scout name is required"),
    fiscalYear: z.string().regex(/^\d{4}-\d{4}$/, "Invalid fiscal year format"),
    amount: z.number().positive("Amount must be positive"),
    paymentMethod: z.enum(["Cash", "Check", "Scout Account"]),
    checkNumber: z
      .string()
      .regex(/^\d+$/, "Check number must be numeric")
      .nullable()
      .optional(),
    date: z.string().datetime(),
